...
function init_paymentsense() {
	class WC_Gateway_Paymentsense extends WC_Payment_Gateway {
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'gateway_hashmethod' => array(
					'title'       => __( 'Gateway Hash Method:', 'woocommerce' ),
					'type'        => 'select',
					'description' => __( 'This is the hash method set in MMS under "Account Admin" > "Account Settings". By default, this will be SHA1.', 'woocommerce' ),
					'default'     => 'SHA1',
					'desc_tip'    => true,
					'options'     => array(
		...
		function get_paymentsense_args( $order ) {
			...
			$HashMethod                = $this->gateway_hashmethod;
			$PreSharedKey              = $this->gateway_presharedkey;
			...
			//die("String: ". $str1 . "Hash: " . $HashMethod . "Key: " . $PreSharedKey);

			$HashDigest = generateStringToHash( $str1, $HashMethod, $PreSharedKey );
		...
		function successful_request( $posted ) {
			...
			//$return_hash = hash( $HashMethod, $return_string );
			$return_hash = generateStringToHash( $return_string, $HashMethod, $this->gateway_presharedkey );
...
function generateStringToHash( $StringToHash, $HashMethod, $PreSharedKey ) {
	$PreSharedKeyBool = null;

	switch ( $HashMethod ) {
		case 'MD5':
		case 'SHA1':
			$PreSharedKeyBool = true;
			break;
		case 'HMACMD5':
		case 'HMACSHA1':
			$PreSharedKeyBool = false;
			break;
	}
	$ReturnString = ( $PreSharedKeyBool == true ? 'PreSharedKey=' . $PreSharedKey . '&' : '' );

	$ReturnString = $ReturnString . $StringToHash;

	switch ( $HashMethod ) {
		case 'MD5':
			$HashDigest = md5( $ReturnString );
			break;
		case 'SHA1':
			$HashDigest = sha1( $ReturnString );
			break;
		case 'HMACMD5':
			$HashDigest = hash_hmac( 'md5', $ReturnString, $PreSharedKey );
			break;
		case 'HMACSHA1':
			$HashDigest = hash_hmac( 'sha1', $ReturnString, $PreSharedKey );
			break;
	}
